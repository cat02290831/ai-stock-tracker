...
def draw_kline_chart(stock_name):
    ...
    try:
        ...
        # 如果發生任何錯誤（包含 Rate limited），顯示警告並切換為備用資料
        st.warning("⚠️ Yahoo 股市 API 流量限制中，已為您自動切換為【備用模擬 K 線圖】以供測試版面。")
    ...
    # 美化圖表外觀
    fig.update_layout(
        title=f"{stock_name} - 近期日線圖 {title_suffix}",
        yaxis_title="股價 (TWD)",
